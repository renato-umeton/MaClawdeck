...
| `attention` | 2 | one needs_input WITH a question and one WITHOUT (the v1 fallback), turn chips, subagent rows. Since v0.31.0 it is also the **effort** fixture (crabd 0.35.0): `high` / `xhigh` / `max` across its rows, one row with **`effort: null`**, one with the key **absent entirely** (a crabd that does not serve it) and one carrying **`ultra`** - a level that does not exist, which must still render as itself, because the widget matches `effort` against no set at all |
| `question` | 3 | long multi-line question, 5 subagent rows on one card and a `+N more` clamp on another; carries `events` and `burn.daily`. It is also the **small-slot** fixture — see the 840x344 note below. Since v0.31.0 it carries `effort` on five of its seven rows, one `null` and one absent, so the badge is exercised on **question cards** as well as plain ones |
...
## v0.31.0 — the effort badge, and the one slot where it costs a second badge row

`sessions[].effort` (crabd 0.35.0, `docs/STATE-CONTRACT.md`) is a fifth card badge and a
fifth sheet chip. It takes the badge metrics unchanged — no new size, no new scale — and
separates from the muted model badge on colour weight (`--text-color` against `--muted`),
the way `.badge-sub` already does. The label is the served string uppercased, clamped at
`EFFORT_LABEL_MAX` = 8 characters because the value is unvalidated text off the wire; the
enum is deliberately absent, so a level this widget has never heard of renders as itself.
Absent or null is NO badge — an unknown effort is not a low one.

**MEASURED 2026-09-09, `?mock=attention` and `?mock=dense`, in headless Chromium. Zero
overflow at every size checked** — every card's `scrollHeight == clientHeight`, and the badge
row's bottom sits **12.52 px** inside the card's own padding on every card at every size:

| slot | fixture | badge row | worst card |
|---|---|---|---|
| 2560x720 (Edge), comfortable | `attention` | 291.46 px | **25 px, one row** — nothing moved |
| 2560x720 (Edge), comfortable | `dense` | 291.47 px | **56 px, two rows** on two cards |
| 2560x720 (Edge), compact | `dense` | — | **19 px, one row** on all of them |
| 1799x720 (3 columns, short) | `attention` | 265.84 px | **25 px, one row** |
| 1680x1050 (browser, 3 columns) | `attention` | 243.83 px | **56 px, two rows** on two cards |
| 1680x1050 (browser, 3 columns) | `dense` | 243.83 px | **56 px, two rows** on one card |

A second badge row is the whole cost, and only on a card already carrying three other chips.
The boundary, at the comfortable chip size: model (`opus-5` 70.88) + FAST (58.43) + `3 sub`
(61.55) is **200.94 px** with the 5.04 px gaps and fits every slot; HIGH (58.43) takes it to
**264.41 px**, which fits 2560x720 and 1799x720 and wraps at 1680x1050. On `dense` at the
Edge slot the ctx chip is in the row too, which is what wraps two of its cards there — below
1800 px `.badge-ctx` is already `display: none`, so it leaves the sum again.

The card absorbs the second row rather than growing: the badges are `flex: 0 0 auto` at the
bottom and the event and repo lines above give it back.

**Not hidden at the narrow slots, deliberately.** Rule 6 hides secondary content when it
would otherwise be cut or shrunk, and nothing here is either — a second badge row inside the
card's own padding is not the wrap the 1800 px `.badge-ctx` rule was written for (that one
grew the card by a line at 720 px of height). If a future slot does overflow, `.badge-effort`
is the next chip to drop after `.badge-ctx`, for the same reason: it annotates the model, it
does not identify the session, and the sheet still carries it.

Fixtures: `attention`, `question` and `dense` carry `effort`; **`normal` deliberately does
not**, because it is the v1 regression and its whole job is that a field crabd has not
learned yet is absent. `attention` is the branch fixture — `high` / `xhigh` / `max`, one row
with **`effort: null`**, one with the key **absent entirely** (a crabd below 0.35.0) and one
carrying **`ultra`**, a level that does not exist, which must still render as itself.
`dense` is the crowded case: all fourteen rows carry one.
