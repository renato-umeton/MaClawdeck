...
- **0.35.0 crabd + 0.31.0 widget (2026-09-09)** - the session cards say what effort a session
  is thinking at. Claude Code records a reasoning effort on every assistant message it writes;
  crabd now serves it as `sessions[].effort` and the panel draws it as a badge beside the model,
  and as a chip in the card's detail sheet. It is served and rendered **verbatim**: the widget
  matches it against no list of known levels, so a level added after this release still shows up
  by name rather than as a blank chip. A session whose transcript has never stated one - an older
  transcript, or a message the CLI wrote itself - gets **no badge at all**, never a guessed level
  and never the one the last session happened to be using. Nothing else moves: `schema` stays 5,
  so an older panel ignores the new field and an older crabd simply gives the new panel nothing
  to draw.
